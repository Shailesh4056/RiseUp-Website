...
class Search
{
	...
	public function mainSearchBar()
	{
		...
		$this->db->query('SELECT * FROM post WHERE Title LIKE :Search OR Article LIKE :Search OR Url LIKE :Search OR Image LIKE :Search OR Channel_Url LIKE :Search AND Channel_Url = :Url ');
		...
		foreach ($Sql_Data as $key => $value) {
			...
			<tr>
					...
					<img src="http://riseup.lit:81/images/'.$value->Image.'">
				</div></td>
						<td><div class="lit-lithome-contant-title"><p style="-webkit-line-clamp:1 !important;" >'.$value->Title.'</p>
