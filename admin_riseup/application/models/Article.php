<?php
	...
	{
			$this->db->query('SELECT Id FROM views WHERE  Post_Url = :Post_Url '.$data);
	...
	{
			$this->db->query('SELECT Id FROM comments WHERE  Post_Url = :Post_Url '.$data);
			$this->db->bind(':Post_Url', $url);
			return count($this->db->resultSet());
	}
	...
	{
			$this->db->query('SELECT Id FROM like_dislike WHERE Like_Dislike = 1 AND Post_Url = :Post_Url '.$data);
			$this->db->bind(':Post_Url', $url);
			return count($this->db->resultSet());
	}
	...
	{
			$this->db->query('SELECT Id FROM like_dislike WHERE Like_Dislike = 0 AND Post_Url = :Post_Url '.$data);
			$this->db->bind(':Post_Url', $url);
			return count($this->db->resultSet());
	}
		public function getTraffic($data)
	{
		$this->db->query('SELECT Source_Type, COUNT(views.Id) AS Views From views WHERE Post_Url = :Post_Url GROUP by Source_Type ORDER BY Views DESC LIMIT 6');
...
';
		...
		{
			$this->db->query('SELECT Comments, Channel_Url,Post_Url,User_Url,C_Date  FROM comments WHERE Post_Url = :Post_Url ORDER BY C_Date DESC');
			$this->db->bind(':Post_Url',$data);
